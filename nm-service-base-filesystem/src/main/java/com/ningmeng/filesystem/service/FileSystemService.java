...
//文件id fileSystem.setFileId(fileId);
// 文件在文件系统中的路径
...
public String fdfs_upload(MultipartFile file) {
    try {
        //加载fdfs的配置 initFdfsConfig(); //创建tracker client
    ...
    return file1; } catch (Exception e) { e.printStackTrace(); }return null; }
...
public ResponseResult saveCoursePic(String courseId, String pic){
    ...
    CoursePic coursePic = null; if(picOptional.isPresent()){
        coursePic = picOptional.get();
    }//没有课程图片则新建对象
    if(coursePic == null){ coursePic = new CoursePic(); }
    coursePic.setCourseid(courseId); coursePic.setPic(pic); //保存课程图片
coursePicRepository.save(coursePic); return new ResponseResult(CommonCode.SUCCESS); }
